...
class Banditron(nn.Module):
    def __init__(self, input_dim=46, output_dim=8, error=0, 
        ...
        self.sparse_rate = sparse_rate
        self.gamma_banditron = gamma_banditron
        self.lr_default = 5e-7 #  loss neurons: 5e-7; electrode shift: 5e-11; drift: 5e-7
        self.lr = self.lr_default
    ...
    def reset(self):
        self.update_count = 0
        self.layer1_weight_update_none_zero_practical = 0
        self.layer2_weight_update_none_zero_practical = 0
        self.layer3_weight_update_none_zero_practical = 0
        self.layer1_weight_update_none_zero_theoretical = 0
        self.layer2_weight_update_none_zero_theoretical = 0
        self.layer3_weight_update_none_zero_theoretical = 0
    ...
    def single_forward(self, x, label_):
        ...
        pred = torch.cat((pred_[:, :int(self.output_dim/2)].clone().t(), pred_[:, int(self.output_dim/2):].clone().t()), dim=1)
        
        if self.update_active:
            self.update_count += 1
            pred = self.update_layer_banditron(x, pred, label_)
    ...
    def forward(self, x, label_):
        ...
        for seq in range(seq_length):
            ...
            pred = self.single_forward(spikes, label_)
    ...
    def update_layer_banditron(self, input_, pred_, label_):
        ...
        if not sparsify:
            dU = torch.zeros_like(self.fc1.weight)
            for out_order in range(label_.shape[1]):
                for r in range(int(self.output_dim//2)):
